...
public class Oauth2proxyConfigClientInstallationProvider implements ClientInstallationProvider {
    ...
    public Response generateInstallation(KeycloakSession session, RealmModel realm, ClientModel client, URI baseUri) {
        ...
        config.append("# OAuth2 Client ID from Keycloak\n");
        ...
        if (showClientCredentialsAdapterConfig(client)) {
            ...
            if (adapterConfig != null && adapterConfig.containsKey("secret")) {
                config.append("# OAuth2 Client Secret from Keycloak\n");
                config.append("# IMPORTANT: Keep this secret secure and never commit it to version control!\n");
        ...
        config.append("# You can add additional scopes if needed (e.g., 'groups', 'roles')\n");
        config.append("scope = \"openid email profile\"\n\n");
        
        config.append("###########################################################\n");
        config.append("## REDIRECT URL\n");
        config.append("###########################################################\n\n");
        ...
        if (redirectUrl.isEmpty()) {
            ...
            config.append("# Verify this matches your OAuth2 Proxy deployment domain\n");
        ...
        config.append("# Set the authorization header with the access token\n");
        config.append("# Use this with pass_authorization_header = true\n");
        config.append("# set_authorization_header = true\n\n");
        
        config.append("# Set X-Auth-Request headers with user information\n");
        config.append("# Useful for Nginx auth_request module or similar\n");
        config.append("# set_xauthrequest = true\n\n");
        
        config.append("###########################################################\n");
        config.append("## REVERSE PROXY CONFIGURATION\n");
        config.append("###########################################################\n\n");
        ...
        config.append("# Example: skip_auth_regex = [\"^/health$\", \"^/public/.*\"]\n");
        config.append("# skip_auth_regex = []\n\n");
        
        config.append("# Whitelist domains for redirects (security feature)\n");
        config.append("# Only allow redirects to these domains after authentication\n");
        config.append("# whitelist_domains = [\".example.com\"]\n\n");
        
        config.append("# Custom headers to pass to upstream\n");
        config.append("# Format: header:value\n");
        config.append("# custom_headers = [\"X-Custom-Header:value\"]\n\n");
